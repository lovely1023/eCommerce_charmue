...
final class Menu_Icons_Picker {
	...
	protected static function _get_menu_item_fields( $meta ) {
		$fields = array_merge(
			array(
				array(
					'id'    => 'type',
					'label' => __( 'Type', 'sparklethemes' ),
					'value' => $meta['type'],
				),
				array(
					'id'    => 'icon',
					'label' => __( 'Icon', 'sparklethemes' ),
					'value' => $meta['icon'],
				),
			),
		...
		?>
			<div class="field-icon description-wide menu-icons-wrap" data-id="<?php echo json_encode( $item->ID ); ?>">
				...
				<p class="description submitbox">
					<label><?php esc_html_e( 'Icon:', 'sparklethemes' ) ?></label>
					<?php printf( '<a class="_select">%s</a>', esc_html__( 'Select', 'sparklethemes' ) ); ?>
					<?php printf( '<a class="_remove submitdelete hidden">%s</a>', esc_html__( 'Remove', 'sparklethemes' ) ); ?>
				</p>
	...
	public static function _columns( $columns ) {
		$columns['icon'] = __( 'Icon', 'sparklethemes' );
