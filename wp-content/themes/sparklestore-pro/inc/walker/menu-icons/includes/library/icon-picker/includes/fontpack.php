...
final class Icon_Picker_Fontpack {
	...
	protected function __construct() {
		...
		$this->messages = array(
			'no_config'    => __( 'Icon Picker: %1$s was not found in %2$s.', 'sparklethemes' ),
			'config_error' => __( 'Icon Picker: %s contains an error or more.', 'sparklethemes' ),
			'invalid'      => __( 'Icon Picker: %1$s is not set or invalid in %2$s.', 'sparklethemes' ),
			'duplicate'    => __( 'Icon Picker: %1$s is already registered. Please check your font pack config file: %2$s.', 'sparklethemes' ),
		);
	...
	protected function get_pack_data( DirectoryIterator $pack_dir ) {
		...
		$pack_data = array(
			'id'             => "pack-{$config['name']}",
			'name'           => sprintf( __( 'Pack: %s', 'sparklethemes' ), $config['name'] ),
