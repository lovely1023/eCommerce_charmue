(function( $, wp_customize ) {
	...
	var is_rtl = SparkleWP_Customizer_Builder.is_rtl;
	...
	var CustomizeBuilder = function( options, id ){
		...
		var Builder = {
			...
			container: null,
			ready: false,
			devices: {'desktop': SparkleWP_Customizer_Builder.desktop_label, 'mobile': SparkleWP_Customizer_Builder.mobile_tablet_label },
			activePanel: 'desktop',
			panels: {},
			activeRow: 'main',
			draggingItem: null,
		...
		drag_drop: function(){
			...
			$( '.sparklewp-device-panel', that.container ).each(
				function(){
					var panel             = $( this );
					var device            = panel.data( 'device' );
					var sortable_ids      = [];
					that.panels[ device ] = {};
					$( '.sparklewp-cb-items', panel ).each(
						function( index ){
					...
					$( '.sparklewp-available-items .grid-stack-item', panel ).draggable(
						{
							revert: 'invalid',
							connectToSortable: ( sidebar_id ) ? '#' + sidebar_id : false,
							start: function( event, ui ){
								   $( 'body' ).addClass( 'builder-item-moving' );
								   $( '.sparklewp-cb-items', panel ).css( 'z-index', '' );
								   ui.helper.parent().css( 'z-index', 9999 );
								   ui.helper.closest( '.sparklewp-available-items' ).addClass( 'spwp-available-item-moving' );
							},
							stop: function(  event, ui ){
								$( 'body' ).removeClass( 'builder-item-moving' );
								$( '.sparklewp-cb-items', panel ).css( 'z-index', '' );
								$( '.sparklewp-available-items', panel ).removeClass( 'spwp-available-item-moving' );
					...
					$( '.sparklewp-available-items .grid-stack-item', panel ).resizable(
						{
							handles: 'w, e',
							stop: function( event, ui ){
			...
			addNewWidget: function ( $item, row ) {
				...
				var that  = this;
				var panel = that.container.find( '.sparklewp-device-panel.sparklewp-panel-' + that.activePanel );
				var el    = row;
				if ( ! _.isObject( el ) ) {
					el = panel.find( '.sparklewp-cb-items' ).first();
				}
			...
			addPanel: function( device ){
				...
				var template_id = 'tmpl-sparklewp-cb-panel';
				...
				return '<div class="sparklewp-device-panel sparklewp-vertical-panel sparklewp-panel-' + device + '" data-device="' + device + '">' + html + '</div>';
			},
			addDevicePanels: function(){
				...
				_.each(
					...
					function( device_name, device ) {
						...
						$( '.sparklewp-cb-devices-switcher', that.container ).append( '<a href="#" class="switch-to switch-to-' + device + '" data-device="' + device + '">' + device_name + '</a>' );
						$( '.sparklewp-cb-body', that.container ).append( panelHTML );
					}
				);
			},
			addItem: function( node ){
				var that        = this;
				var template    = that.getTemplate();
				var template_id = 'tmpl-sparklewp-cb-item';
				if (  $( '#' + template_id ).length == 0 ) {
					return;
				}
			...
			addAvailableItems: function(){
				...
				_.each(
					that.devices,
					function(device_name, device ){
						var $itemWrapper = $( '<div class="sparklewp-available-items" data-device="' + device + '"></div>' );
						$( '.sparklewp-panel-' + device, that.container ).append( $itemWrapper );
						$itemWrapper.append( '<div class="spwp-close-item-wrap"><span class="spwp-close-item-panel dashicons dashicons-no"></span></div>' );
			...
			switchToDevice: function( device, toggle_button ){
				...
				if ( numberDevices > 1 ) {
					$( '.sparklewp-cb-devices-switcher a', that.container ).removeClass( 'sparklewp-tab-active' );
					$( '.sparklewp-cb-devices-switcher .switch-to-' + device, that.container ).addClass( 'sparklewp-tab-active' );
					$( '.sparklewp-device-panel', that.container ).addClass( 'sparklewp-panel-hide' );
					$( '.sparklewp-device-panel.sparklewp-panel-' + device, that.container ).removeClass( 'sparklewp-panel-hide' );
					that.activePanel = device;
				} else {
					$( '.sparklewp-cb-devices-switcher a', that.container ).addClass( 'sparklewp-tab-active' );
				}
				...
				if ( _.isUndefined( toggle_button ) || toggle_button ) {
					...
					/*                 else if ( device == 'all' ) {
						wp_customize.section( 'sparklewp-menu-icon-sidebar-section' ).focus();
					}*/
				}
			...
			addExistingRowsItems: function(){
				...
				_.each(
					...
					function( $rows,  device ) {
						...
						_.each(
							device_data,
							function( items, row_id ) {
								if ( ! _.isUndefined( items ) ) {
									_.each(
										items,
										function (node, index) {
											var item = $( '.sparklewp-available-items[data-device="' + device + '"] .grid-stack-item[data-id="' + node.id + '"]' ).first();
			...
			focus: function(){
				this.container.on(
					'click',
					'.sparklewp-cb-item-setting, .sparklewp-cb-item-name, .item-tooltip',
					function( e ) {
						e.preventDefault();
			...
			remove: function(){
				...
				$document.on(
					'click',
					'.sparklewp-device-panel .sparklewp-cb-item-remove',
					function ( e ) {
						e.preventDefault();
						var item  = $( this ).closest( '.grid-stack-item' );
						var panel = item.closest( '.sparklewp-device-panel' );
						item.attr( 'data-gs-width', 1 );
						item.attr( 'data-gs-x', 0 );
						item.removeAttr( 'style' );
						$( '.sparklewp-available-items', panel ).append( item );
						that.updateAllGrids();
						that.save();
					}
				);
			...
			hidePanel: function(){
				this.container.removeClass( 'sparkle-builder-show' );
				spwp_hide_item_panel( this.container.find( '.sparklewp-available-items' ) );
				$( '#customize-preview' ).removeClass( 'cb--preview-panel-show' ).removeAttr( 'style' );
			},
			showPanel: function(){
				var that = this;
				this.container.removeClass( 'sparkle-builder--hide' ).addClass( 'sparkle-builder-show' );
				setTimeout(
					function(){
			...
			togglePanel: function(){
				...
				wp_customize.state( 'expandedPanel' ).bind(
					function( paneVisible ) {
						if ( wp_customize.panel( options.panel ).expanded() ) {
							  $document.trigger( 'sparklewp_panel_builder_open', [ options.panel ] );
				...
				that.container.on(
					'click',
					'.sparklewp-panel-close',
					function(e){
						e.preventDefault();
						that.container.toggleClass( 'sparkle-builder--hide' );
						if ( that.container.hasClass( 'sparkle-builder--hide' ) ) {
			...
			panelLayoutCSS: function(){
				...
				if ( is_rtl ) {
					this.container.find( '.sparklewp-cb-inner' ).css( {'margin-right': sidebarWidth } );
				} else {
					this.container.find( '.sparklewp-cb-inner' ).css( {'margin-left': sidebarWidth } );
				}
			...
			init: function( controlId, items, devices ){
				...
				var template    = that.getTemplate();
				var template_id = 'tmpl-sparkle-builder-panel';
				...
				if ( options.section ) {
					wp_customize.section( options.section ).container.addClass( 'sparkle-hide' );
				}
				...
				that.container.on(
					'click',
					'.sparklewp-cb-devices-switcher a.switch-to',
					function(e){
						e.preventDefault();
						var device = $( this ).data( 'device' );
						that.switchToDevice( device );
						$( '.sparklewp-cb-body' ).find( '.sparklewp-available-items' ).each(
							function () {
								spwp_hide_item_panel( $( this ) );
							}
						)
					}
				);

				$document.trigger( 'sparklewp_builder_panel_loaded', [ id, that ] );
	...
	wp_customize.bind(
		...
		function( e, b ) {
			_.each(
				SparkleWP_Customizer_Builder.builders,
				function( opts, id ){
					...
					setTimeout(
						function(){
							if ( $( '#sub-accordion-panel-widgets .no-widget-areas-rendered-notice .footer_moved_widgets_text' ).length ) {
								...
								$( '#sub-accordion-panel-widgets .no-widget-areas-rendered-notice' ).append( '<p class="footer_moved_widgets_text">' + SparkleWP_Customizer_Builder.footer_moved_widgets_text + '</p>' );
							}
			...
			wp_customize.state( 'expandedSection' ).bind(
				function( section ) {
					$( '.sparklewp-device-panel .grid-stack-item' ).removeClass( 'item-active' );
					$( '.sparklewp-cb-row' ).removeClass( 'row-active' );
					if ( section ) {
						  $( '.sparklewp-cb-row[data-id="' + section.id + '"]' ).addClass( 'row-active' );
						  $( '.sparklewp-device-panel .grid-stack-item.for-s-' + section.id ).addClass( 'item-active' );
					}
				}
			);
	...
	$document.on(
		'mouseover',
		'.sparklewp-cb-row .grid-stack-item',
		function( e ) {
			var item  = $( this );
			var nameW = $( '.sparklewp-cb-item-remove',item ).outerWidth() + $( '.sparklewp-cb-item-setting',item ).outerWidth();
	...
	$document.on(
		...
		function( e ) {
			...
			this_item_wrap   = this_add_new.next( '.sparklewp-available-items' );
			if ( ! this_item_wrap.length ) {
				this_item_wrap = this_add_new.closest( '.sparklewp-cp-sidebar' ).next( '.sparklewp-available-items' );
			}
			if ( this_item_wrap.length ) {
				this_item_wrap.toggleClass( 'spwp-show-items' );
				$( 'body' ).toggleClass( 'spwp-body-overlay' );
				this_add_new.toggleClass( 'spwp-hide-items' );
			}

		}
	);
	function spwp_hide_item_panel(this_item_wrap){

		var this_add_new = this_item_wrap.prev( '.spwp-add-new-item' );
		if ( ! this_add_new.length ) {
			this_add_new = this_add_new.prev( '.sparklewp-cp-sidebar' ).find( '.spwp-add-new-item' );
		}
		if ( this_item_wrap.length ) {
			this_item_wrap.removeClass( 'spwp-show-items' );
			$( 'body' ).removeClass( 'spwp-body-overlay' );
			this_add_new.removeClass( 'spwp-hide-items' );
		}

	}
	$document.on(
		'click',
		'.spwp-close-item-panel',
		function( e ) {
			var this_close_item_panel = $( this ),
			this_item_wrap            = this_close_item_panel.closest( '.sparklewp-available-items' );

			spwp_hide_item_panel( this_item_wrap );
