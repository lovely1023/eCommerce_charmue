    ...
    class Jet_Elements_Integration {
        ...
        public function init() {
            ...
            $this->localize_data['messages'] = array(
                'invalidMail' => esc_html__('Please specify a valid e-mail', 'sparklestore-pro'),
            );
        }
        ...
        public function editor_styles() {
            ...
            wp_enqueue_style(
                    'jet-elements-font', jet_elements()->plugin_url('assets/css/lib/jetelements-font/css/jetelements.css'), array(), jet_elements()->get_version()
            );
        }
        ...
        public function enqueue_scripts() {
            wp_enqueue_script(
                    'sparklestore-pro', jet_elements()->plugin_url('assets/js/jet-elements.js'), array('jquery', 'elementor-frontend'), jet_elements()->get_version(), true
            );

            wp_localize_script(
                    'sparklestore-pro', 'jetElements', apply_filters('jet-elements/frontend/localize-data', $this->localize_data)
            );
        }
        ...
        public function editor_scripts() {
            wp_enqueue_script(
                    'jet-elements-editor', jet_elements()->plugin_url('assets/js/jet-elements-editor.js'), array('jquery'), jet_elements()->get_version(), true
            );
        }
        ...
        public function register_addons($widgets_manager) {
            ...
            $avaliable_widgets = of_get_option('avaliable_widgets');
        ...
        public function register_vendor_addons($widgets_manager) {
            ...
            $allowed_vendors = apply_filters(
                    'jet-elements/allowed-vendor-addons', array(
                'smartslider3' => array(
            ...
            foreach ($allowed_vendors as $vendor) {
                if (is_callable($vendor['conditional']['cb']) && true === call_user_func($vendor['conditional']['cb'], $vendor['conditional']['arg'])) {
        ...
        public function include_control($class_name, $grouped = false) {
            ...
            $filename = sprintf(
                    'includes/controls/%2$sclass-%1$s.php', str_replace('_', '-', strtolower($class_name)), ( true === $grouped ? 'groups/' : '')
            );
        ...
        public function register_category() {
            ...
            $elements_manager->add_category(
                    $cherry_cat, array(
                'title' => esc_html__('Sparkle Pro Elements', 'sparklestore-pro'),
                'icon' => 'font',
                    ), 1
            );
        }
