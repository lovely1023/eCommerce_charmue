...
final class Menu_Icons_Picker {
	...
	protected static function _get_menu_item_fields( $meta ) {
		$fields = array_merge(
			array(
				array(
					'id'    => 'type',
					'label' => __( 'Type', 'sparklestore-pro' ),
					'value' => $meta['type'],
				),
				array(
					'id'    => 'icon',
					'label' => __( 'Icon', 'sparklestore-pro' ),
					'value' => $meta['icon'],
				),
			),
		...
		?>
			<div class="field-icon description-wide menu-icons-wrap" data-id="<?php echo json_encode( $item->ID ); ?>">
				...
				<p class="description submitbox">
					<label><?php esc_html_e( 'Icon:', 'sparklestore-pro' ) ?></label>
					<?php printf( '<a class="_select">%s</a>', esc_html__( 'Select', 'sparklestore-pro' ) ); ?>
					<?php printf( '<a class="_remove submitdelete hidden">%s</a>', esc_html__( 'Remove', 'sparklestore-pro' ) ); ?>
				</p>
	...
	public static function _columns( $columns ) {
		$columns['icon'] = __( 'Icon', 'sparklestore-pro' );
